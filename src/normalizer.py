...
class DataNormalizer:
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(
            "DataNormalizer initialised  input=%s  output=%s  eps=%.1e",
            self.input_dir,
            self.output_dir,
            self.eps,
        )
        ...
        *,
        ...
        for fpath in files:
            try:
                ...
                raise RuntimeError(f"Bad JSON {fpath.name}: {exc}") from exc
            ...
            for k, v in prof.items():
                ...
                if isinstance(v, list) and all(
                    ...
                    value_buf.setdefault(k, []).append(
                        torch.tensor(v, dtype=torch.float32)
                    )

        # map each key to a method -------------------------------------
        ...
        for key, tensors in value_buf.items():
            ...
            stats[key] = self._compute_stats_for_key(
                vec, method, symlog_percentile, (symlog_thresholds or {}).get(key)
            )
    ...
    def _compute_stats_for_key(
        ...
        if method == "iqr":
            ...
            centre = 0.0 if (gmin < 0 < gmax) else (gmin + gmax) / 2.0
            scale = max((gmax - gmin) / 2.0, eps)
            stats.update(
                center=centre, scale=scale, alpha=math.tan(0.99 * math.pi / 2)
            )
    ...
    def denormalize(
        ...
        is_scalar = not isinstance(v, (Tensor, list))
    ...
    def process_profiles(self, stats: Dict[str, Any]) -> None:
        ...
        for fpath in self.input_dir.glob("*.json"):
            ...
            try:
                ...
                logger.error("Skipping bad JSON %s: %s", fpath.name, exc)
                continue
            ...
            for k, v in prof.items():
                ...
                if key_stats is None:
                    logger.warning(
                        "No stats for key '%s' (method=%s); keeping raw", k, meth
                    )
                    out_prof[k] = v
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to normalize %s in %s: %s; keeping raw",
                        k,
                        fpath.name,
                        exc,
                    )
