...
* Robust JSON serialisation for numpy / torch
...
def setup_logging(level: int = logging.INFO, log_file: Optional[Union[str, Path]] = None) -> None:
    ...
    # remove any stale handlers (idempotent re-calls)
    while root.handlers:
        h = root.handlers.pop()
        h.close()
    ...
    if log_file:
        try:
            ...
            f_handler = logging.FileHandler(file_path, mode="a", encoding="utf-8")
...
def load_config(path: Union[str, Path]) -> Optional[Dict[str, Any]]:
    ...
    try:
        cfg = json5.loads(raw) if _HAS_JSON5 else json.loads(_strip_jsonc(raw))
...
def validate_config(cfg: Dict[str, Any]) -> bool:
    ...
    # lists that must exist and be non-empty
    ...
    if not isinstance(seq_len, dict) or out_seq not in seq_len or not isinstance(seq_len[out_seq], int) or seq_len[out_seq] <= 0:
        _err("'sequence_lengths' must map each sequence type to a positive int; missing or invalid for output sequence.")
    ...
    # quick sanity for a few other ints / floats
...
def _json_encoder(obj: Any):
    """Helper for json.dump to handle numpy / torch."""
...
def save_json(data: Dict[str, Any], path: Union[str, Path]) -> bool:
    """Write *data* to *path* with indent and robust encoding."""
    p = Path(path)
    try:
